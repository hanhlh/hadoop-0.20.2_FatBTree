...
public class KosmosFileSystem extends FileSystem {
    ...
    @Deprecated
    public boolean isDirectory(Path path) throws IOException, MessageException {
	Path absolute = makeAbsolute(path);
        String srep = absolute.toUri().getPath();
    ...
    @Deprecated
    public boolean isFile(Path path) throws IOException, MessageException {
	Path absolute = makeAbsolute(path);
        String srep = absolute.toUri().getPath();
        return kfsImpl.isFile(srep);
    }

    public FileStatus[] listStatus(Path path) throws IOException, MessageException {
        Path absolute = makeAbsolute(path);
        String srep = absolute.toUri().getPath();
    ...
    public FileStatus getFileStatus(Path path) throws IOException, MessageException {
	Path absolute = makeAbsolute(path);
        String srep = absolute.toUri().getPath();
    ...
    public FSDataInputStream open(Path path, int bufferSize) throws IOException, MessageException {
        if (!exists(path))
    ...
    public boolean delete(Path path, boolean recursive) throws IOException, MessageException {
      Path absolute = makeAbsolute(path);
      String srep = absolute.toUri().getPath();
      if (kfsImpl.isFile(srep))
    ...
    @Deprecated
    public boolean delete(Path path) throws IOException, MessageException {
      return delete(path, true);
    }

    @Deprecated
    public long getLength(Path path) throws IOException, MessageException {
	Path absolute = makeAbsolute(path);
        String srep = absolute.toUri().getPath();
    ...
    public void copyFromLocalFile(boolean delSrc, Path src, Path dst) throws IOException, MessageException, ServiceException {
	FileUtil.copy(localFs, src, this, dst, delSrc, getConf());
    }

    public void copyToLocalFile(boolean delSrc, Path src, Path dst) throws IOException, MessageException, ServiceException {
