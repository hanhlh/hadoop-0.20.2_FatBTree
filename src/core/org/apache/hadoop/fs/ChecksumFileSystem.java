...
public abstract class ChecksumFileSystem extends FilterFileSystem {
  ...
  private static class ChecksumFSInputChecker extends FSInputChecker {
    ...
    protected int readChunk(long pos, byte[] buf, int offset, int len,
        byte[] checksum) throws IOException, MessageException {
      boolean eof = false;
      if(needChecksum()) {
        try {
    ...
    private long getFileLength() throws IOException, MessageException {
      if( fileLen==-1L ) {
 ...
 * @throws MessageException
  ...
  public boolean reportChecksumFailure(Path f, FSDataInputStream in,
                                       long inPos, FSDataInputStream sums, long sumsPos) throws MessageException {
    return false;
  }
}
