...
public class RPC {
  ...
  static private class ClientCache {
    ...
    private void stopClient(Client client) {
    	System.out.println("RPC.stopClient()");
      synchronized (this) {
        client.decCount();
  ...
  static VersionedProtocol waitForProxy(Class protocol,
	 ...
	 NameNode.LOG.info("RPC.waitForProxy");
    ...
    while (true) {
      ...
      // wait for retry
      try {
    	  NameNode.LOG.info("RPC.waitForProxy wait for retry");
        Thread.sleep(1000);
  ...
  public static VersionedProtocol getProxy(Class<?> protocol,
	  ...
	  NameNode.LOG.info("RPC.getProxy");
	  Invoker invoker = new Invoker(addr, ticket, conf, factory);
	  ClassLoader classLoader = protocol.getClassLoader();
	  Class[] protocolClass = new Class[] {protocol};
	  VersionedProtocol proxy =
        (VersionedProtocol) Proxy.newProxyInstance(
            classLoader, protocolClass,
            invoker);
	  NameNode.LOG.info("RPC.getProxy proxy "+proxy);
