...
public abstract class Server {
  ...
  private class Listener extends Thread {
    ...
    private void cleanupConnections(boolean force) {
    	//System.out.println("Server.cleanupConnections()");
    ...
    public void run() {
      ...
      while (running) {
        try {
          ...
          //
          LOG.warn("Out of Memory in server select", e);
          LOG.info("Out of Memory in server select", e);
          try { Thread.sleep(60000); } catch (Exception ie) {}
        } catch (Exception e) {
          LOG.warn("Exception in Responder " +
                   StringUtils.stringifyException(e));
          LOG.info("Exception in Responder " +
                  StringUtils.stringifyException(e));
        }
      }
      LOG.info("Stopping " + this.getName());
  ...
  private void closeConnection(Connection connection) {
	  //System.out.println("Server.closeConnnection");
    synchronized (connectionList) {
  ...
  public synchronized void stop() {
    ...
    if (handlers != null) {
      for (int i = 0; i < handlerCount; i++) {
    	  System.out.println("Stopping server "+i+ " on " + port);
        if (handlers[i] != null) {
