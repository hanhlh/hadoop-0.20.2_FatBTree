...
public class TestCollect extends TestCase
{
          ...
          for (int j = 1; j <= NUM_COLLECTS_PER_THREAD; j++) {
            try {
              out.collect(new IntWritable((id * NUM_COLLECTS_PER_THREAD) + j),
                                          new IntWritable(0));
            } catch (IOException ioe) { } catch (MessageException e) {
  ...
  public void configure(JobConf conf) throws IOException, MessageException {
  ...
  public void testCollect() throws IOException, MessageException, ServiceException {
