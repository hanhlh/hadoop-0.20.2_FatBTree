...
public class TestChainMapReduce extends HadoopTestCase {
  ...
  private static void cleanFlags(JobConf conf) throws IOException, MessageException {
    FileSystem fs = FileSystem.get(conf);
    fs.delete(getFlagDir(conf.getBoolean("localFS", true)), true);
    fs.mkdirs(getFlagDir(conf.getBoolean("localFS", true)));
  }

  private static void writeFlag(JobConf conf, String flag) throws IOException, MessageException, ServiceException {
    FileSystem fs = FileSystem.get(conf);
  ...
  private static boolean getFlag(JobConf conf, String flag) throws IOException, MessageException {
    FileSystem fs = FileSystem.get(conf);
    return fs
  ...
  public static class IDMap
    ...
    public void configure(JobConf conf) throws MessageException, ServiceException {
      this.conf = conf;
