...
public class TestLocalDirAllocator extends TestCase {
  ...
  private static void rmBufferDirs() throws IOException, MessageException {
    assertTrue(!localFs.exists(BUFFER_PATH_ROOT) ||
        localFs.delete(BUFFER_PATH_ROOT));
  }

  private void validateTempDirCreation(int i) throws IOException, MessageException {
  ...
  private File createTempFile() throws IOException, MessageException {
    File result = dirAllocator.createTmpFileForWrite(FILENAME, -1, conf);
    result.delete();
    return result;
  }

  private File createTempFile(long size) throws IOException, MessageException {
