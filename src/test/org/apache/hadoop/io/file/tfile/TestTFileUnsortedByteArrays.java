...
public class TestTFileUnsortedByteArrays extends TestCase {
  ...
  public void testFailureScannerWithKeys() throws IOException, MessageException {
    Reader reader =
  ...
  // we still can scan records in an unsorted TFile
  public void testScan() throws IOException, MessageException {
    Reader reader =
        new Reader(fs.open(path), fs.getFileStatus(path).getLen(), conf);
    Assert.assertFalse(reader.isSorted());
    Assert.assertEquals((int) reader.getEntryCount(), 4);
  ...
  // we still can scan records in an unsorted TFile
  public void testScanRange() throws IOException, MessageException {
    Reader reader =
        new Reader(fs.open(path), fs.getFileStatus(path).getLen(), conf);
    Assert.assertFalse(reader.isSorted());
    Assert.assertEquals((int) reader.getEntryCount(), 4);
  ...
  public void testFailureSeek() throws IOException, MessageException {
    Reader reader =
        new Reader(fs.open(path), fs.getFileStatus(path).getLen(), conf);
    Scanner scanner = reader.createScanner();
