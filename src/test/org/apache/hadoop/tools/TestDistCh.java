...
public class TestDistCh extends junit.framework.TestCase {
  ...
  static class FileTree {
    ...
    Path createSmallFile(Path dir) throws IOException, MessageException, ServiceException {
    ...
    Path mkdir(Path dir) throws IOException, MessageException {
      assertTrue(fs.mkdirs(dir));
      assertTrue(fs.getFileStatus(dir).isDir());
      return dir;
    }

    FileTree(FileSystem fs, String name) throws IOException, MessageException, ServiceException {
      this.fs = fs;
      this.root = "/test/" + name;
