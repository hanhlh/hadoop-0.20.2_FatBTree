...
//import org.apache.hadoop.hdfs.server.namenode.NameNodeFBT;
 ...
 * to communicate with a NameNodeFBT daemon, and connects
...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  public ArrayList<ClientProtocol> namenodes;
  private final ClientProtocol rpcNamenode;
  private ArrayList<ClientProtocol> rpcNamenodes;
  //public final ClientProtocol namenodeDatanode;
  //private final ClientProtocol rpcNamenodeDatanode;
  ...
  public final static String[] NAMESPACE_ADDRESSLIST = {"192.168.0.117",
  														"192.168.0.118",
  														"192.168.0.119",
  														"192.168.0.120"};
														//"192.168.0.102"};
  /**
   * Randomly choose one of datanodes that can do namenode funcionalities
   * */
  public static ClientProtocol createNamenodeDatanode(Configuration conf)
  							throws IOException {

	  return createNamenode(getDatanodeImplementNamenode(conf) ,conf);
  }

  public static ArrayList<ClientProtocol> createNamenodeDatanodes(
		  	Configuration conf) throws IOException {
	  ArrayList<InetSocketAddress> isas = getDatanodesImplementNamenode(conf);
	  ArrayList<ClientProtocol> cps = new ArrayList<ClientProtocol>();
	  for (InetSocketAddress isa:isas) {
		  ClientProtocol cp = createNamenode(isa, conf);
		  cps.add(cp);
	  }
	  return cps;
  }
  ...
  private static ArrayList<ClientProtocol> createRPCNamenodes(
						  ArrayList<InetSocketAddress> nameNodeAddrs,
					      Configuration conf, UnixUserGroupInformation ugi)
	    throws IOException {
	  ArrayList<ClientProtocol> rpcNamenodes = new ArrayList<ClientProtocol>();
	  for (InetSocketAddress namenodeAddr:nameNodeAddrs) {
	    rpcNamenodes.add((ClientProtocol)RPC.getProxy(ClientProtocol.class,
	        ClientProtocol.versionID, namenodeAddr, ugi, conf,
	        NetUtils.getSocketFactory(conf, ClientProtocol.class)));
	  }
	  return rpcNamenodes;
  ...
  private static ArrayList<ClientProtocol> createNamenodes(
		  ArrayList<ClientProtocol> rpcNamenodes)
  						throws IOException {
	  ArrayList<ClientProtocol> namenodes = new ArrayList<ClientProtocol>();

	  for (ClientProtocol rpcNamenode:rpcNamenodes) {
		  RetryPolicy createPolicy = RetryPolicies.retryUpToMaximumCountWithFixedSleep(
		      5, LEASE_SOFTLIMIT_PERIOD, TimeUnit.MILLISECONDS);
		  ...
		  methodNameToPolicyMap.put("create"+rpcNamenode.toString(), methodPolicy);

		  namenodes.add((ClientProtocol) RetryProxy.create(ClientProtocol.class,
		      rpcNamenode, methodNameToPolicyMap));
	  }
	  return namenodes;
}
  ...
  /**
   * Same as this(NameNodeFBT.getAddress(conf), conf);
   * @see #DFSClient(InetSocketAddress, Configuration)
   */
  public DFSClient(Configuration conf) throws IOException {
    //this(NameNodeFBT.getAddress(conf), conf);
  	  //this(getDatanodeImplementNamenode(conf), conf);
	  this(getDatanodesImplementNamenode(conf), conf);
  }

  static InetSocketAddress getDatanodeImplementNamenode(Configuration conf) {
	  ArrayList<URI> namenodeURIList = FileSystem.getDefaultUris(conf);
	  Random random = new Random();
	  int namenodeURIIndex = random.nextInt(namenodeURIList.size());

	  InetSocketAddress isa = NetUtils.createSocketAddr(
			  					namenodeURIList.get(namenodeURIIndex).getAuthority(),
			  					DataNode.DEFAULT_PORT);
	  return isa;
  }

  static ArrayList<InetSocketAddress> getDatanodesImplementNamenode (
		  									Configuration conf) {
	  ArrayList<InetSocketAddress> isas = new ArrayList<InetSocketAddress>();
	  ArrayList<URI> namenodeURIList = FileSystem.getDefaultUris(conf);
	  for (URI uri:namenodeURIList) {
		  InetSocketAddress isa = NetUtils.createSocketAddr(
				  				uri.getAuthority(),
				  				DataNode.DEFAULT_PORT);
		  isas.add(isa);
   ...
   */
  public DFSClient(ArrayList<InetSocketAddress> isaList, Configuration conf
      ) throws IOException {
    this(isaList, conf, null);
  }

  public DFSClient(InetSocketAddress isa, Configuration conf) throws IOException {
	  this(isa, conf, null);
  }
  /**
   ...
   */
  public DFSClient(ArrayList<InetSocketAddress> nameNodeAddrs, Configuration conf,
                   FileSystem.Statistics stats)
    throws IOException {
    this(nameNodeAddrs, null, conf, stats);
  }
  ...
  DFSClient(ArrayList<InetSocketAddress> nameNodeAddrs,
		  ArrayList<ClientProtocol> rpcNamenodes,
    ...
    if (nameNodeAddrs != null && rpcNamenodes == null) {
	      this.rpcNamenodes = createRPCNamenodes(nameNodeAddrs, conf, ugi);
	      this.namenodes = createNamenodes(rpcNamenodes);
		} else if (nameNodeAddrs == null && rpcNamenodes!= null) {
    ...
    this.namenode = this.namenodes.get(0);
    this.rpcNamenode = this.rpcNamenodes.get(0);
  }
  ...
  DFSClient(InetSocketAddress nameNodeAddr,
    ...
    /*int nameNodeAddrIdx = new Random().nextInt(NAMESPACE_ADDRESSLIST.length);
    nameNodeAddrIdx = 0;
    InetSocketAddress nameNodeAddrRandom = new InetSocketAddress(
    									NAMESPACE_ADDRESSLIST[nameNodeAddrIdx],
    									NameNodeFBT.DEFAULT_PORT);
    System.out.println("nameNodeAddrRandom "+nameNodeAddrRandom.toString());*/
    System.out.println("nameNodeAddr "+nameNodeAddr.toString());
    if (nameNodeAddr != null && rpcNamenode == null) {
	      this.rpcNamenode = createRPCNamenode(nameNodeAddr, conf, ugi);
    	//this.rpcNamenode = createRPCNamenode(nameNodeAddrRandom, conf, ugi);
	    this.namenode = createNamenode(this.rpcNamenode);
	    //System.out.println("namenode "+this.namenode);

	} else if (nameNodeAddr == null && rpcNamenodes != null) {
		      //This case is used for testing.
      this.namenode = this.rpcNamenode = rpcNamenode;
    } else {
      throw new IllegalArgumentException(
    ...
    System.out.println("DFSClient initialized");
    //this.namenodes.add(namenode);
    //this.rpcNamenodes.add(rpcNamenode);
  }
 ...
 * @deprecated Use getBlockLocations instead
  ...
  private static LocatedBlocks callGetBlockLocations(ClientProtocol namenode,
      String src, long start, long length) throws IOException, MessageException {
    try {
 ...
 * @throws MessageException
  ...
  public DFSInputStream open(String src) throws IOException, MessageException {
 ...
 * @throws MessageException
  ...
  public boolean setReplication(String src,
	  ...
	  System.out.println("DFSClient.setReplication()");
    try {
 ...
 * @throws MessageException
   */
  public boolean exists(String src) throws IOException, MessageException {
    checkOpen();
 ...
 * @deprecated Use getFileStatus() instead */
  @Deprecated
  public boolean isDirectory(String src) throws IOException, MessageException {
  ...
  public FileStatus getFileInfo(String src, boolean isDirectory) throws IOException,
  			MessageException {
		checkOpen();
		//System.out.println("DFSClient getFileInfo src "+src);
	    try {
	      return namenode.getFileInfo(src, isDirectory);
	    } catch(RemoteException re) {
	      throw re.unwrapRemoteException(AccessControlException.class);
	    }
	}
 ...
 * @throws MessageException
   ...
   */
  MD5MD5CRC32FileChecksum getFileChecksum(String src) throws IOException, MessageException {
    checkOpen();
 ...
 * @throws MessageException
   */
  public boolean mkdirs(String src) throws IOException, MessageException {
 ...
 * @throws ServiceException
  ...
  public boolean synchronizeRootNodes() throws IOException, ServiceException, MessageException {
	  System.out.println("DFSClient.synchronizeRootNodes");
	  ArrayList<InetSocketAddress> nameNodeAddrs = new ArrayList<InetSocketAddress>();
	  for (String nameNodeAddr: NAMESPACE_ADDRESSLIST) {
		  InetSocketAddress isa = new InetSocketAddress(nameNodeAddr, NameNode.DEFAULT_PORT);
		  nameNodeAddrs.add(isa);
	  }
	  this.rpcNamenodes = createRPCNamenodes(nameNodeAddrs, conf, ugi);
      this.namenodes = createNamenodes(rpcNamenodes);
	  for (ClientProtocol namenode:namenodes) {
		  if (!namenode.synchronizeRootNodes())
			  return false;
	  }
	  return true;
  }
 ...
 * @throws MessageException
   ...
   */
  public boolean mkdirs(String src, FsPermission permission)throws IOException, MessageException{
    checkOpen();
    if (permission == null) {
      permission = FsPermission.getDefault();
    }
  ...
  public boolean mkdirs(String src, FsPermission permission, boolean isDirectory)
  												throws IOException, MessageException{
	    checkOpen();
	    if (permission == null) {
	      permission = FsPermission.getDefault();
	    ...
	    LOG.debug(src + ": masked=" + masked);
	    try {
	      return namenode.mkdirs(src, masked, isDirectory);
	    } catch(RemoteException re) {
	      throw re.unwrapRemoteException(AccessControlException.class,
	                                     NSQuotaExceededException.class,
	                                     DSQuotaExceededException.class);
 ...
 * @throws MessageException
  ...
  class DFSInputStream extends FSInputStream {
     ...
     * @throws MessageException
     */
    synchronized void openInfo() throws IOException, MessageException {
     ...
     * @throws MessageException
     */
    synchronized List<LocatedBlock> getAllBlocks() throws IOException, MessageException {
     ...
     * @throws MessageException
     */
    private LocatedBlock getBlockAt(long offset) throws IOException, MessageException {
     ...
     * @throws MessageException
     */
    private synchronized DatanodeInfo blockSeekTo(long target) throws IOException, MessageException {
    ...
    private void fetchBlockByteRange(LocatedBlock block, long start,
                                     long end, byte[] buf, int offset) throws IOException, MessageException {
      //
      // Connect to best DataNode for desired Block, with potential offset
      //
      Socket dn = null;
     ...
     */
    @Override
    public synchronized boolean seekToNewSource(long targetPos) throws IOException, MessageException {
  ...
  ****************************************************************/
  class DFSOutputStream extends FSOutputSummer implements Syncable {
          ...
          synchronized (dataQueue) {
            ...
            try {
              ...
              if (blockStream == null) {
                ...
                //System.out.println("DFSClient 2536 src "+src);
                nodes = nextBlockOutputStream(src);
                //System.out.println("DFSClient 2538");
      ...
      while (!success && clientRunning) {
        ...
        try {
          ...
          return true;          // sleep when we return from here
        	...
        	System.out.println("DFSClient.processDataError");
    ...
    DFSOutputStream(String src, FsPermission masked, boolean overwrite,
        ...
        int buffersize, int bytesPerChecksum) throws IOException, MessageException, ServiceException {
      ...
      try {
       success = namenode.create(
            src, masked, clientName, overwrite, replication, blockSize);
       //NameNode.LOG.info("create "+src+" success is "+success);
      } catch(RemoteException re) {
        throw re.unwrapRemoteException(AccessControlException.class,
                                       NSQuotaExceededException.class,
                                       DSQuotaExceededException.class);
     ...
     * @throws MessageException
     */
    private DatanodeInfo[] nextBlockOutputStream(String client) throws IOException, MessageException {
    ...
    private LocatedBlock locateFollowingBlock(long start
      ...
      while (true) {
        ...
        while (true) {
          try {
        	  /*System.out.println("DFSClien.locateFollowingBlock call" +
        	  		" namenode.addBlock for "+src);*/
    ...
    private void closeThreads() throws IOException {
    	//System.out.println("DFSClient.closeThreads()");
      try {
        streamer.close();
        streamer.join();
     ...
     * @throws MessageException
     */
    private synchronized void closeInternal() throws IOException, MessageException {
      checkOpen();
      isClosed();
  ...
  public boolean transferNamespace(String targetMachine) throws IOException, ClassNotFoundException {
	  StringUtility.debugSpace("DFSClient.transferNamespace");
  ...
  public boolean resetOffloading() throws IOException, ClassNotFoundException {
	  StringUtility.debugSpace("DFSClient.resetOffloading");
