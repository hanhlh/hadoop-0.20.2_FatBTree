...
class DataXceiver implements Runnable, FSConstants {
  ...
  public void run() {
    ...
    try {
      ...
      case DataTransferProtocol.OP_WRITE_BLOCK:
        ...
        break;
      case DataTransferProtocol.OP_WRITE_BLOCKS:
          writeBlocks( in );
          datanode.myMetrics.writeBlockOp.inc(DataNode.now() - startTime);
          if (local)
            datanode.myMetrics.writesFromLocalClient.inc();
   ...
   * @throws IOException
   */
  private void writeBlocks(DataInputStream in) throws IOException {
    DatanodeInfo srcDataNode = null;
    LOG.debug("writeBlock receive buf size " + s.getReceiveBufferSize() +
              " tcp no delay " + s.getTcpNoDelay());
    ...
    LOG.info("DataXceiver writeBlocks,"+numBlocks);
    //
    // Read in the header
    // add in.readBoolean
    //
    ...
    for (int index=0; index<numBlocks; index++) {
    	block[index] = new Block(in.readLong(),
        dataXceiverServer.estimateBlockSize, in.readLong());
    	LOG.info("Receiving block " + block +
    			" src: " + remoteAddress +
    			" dest: " + localAddress);
	    ...
	    try {
	    	// open a block receiver and check if the block does not exist
	    	blockReceiver = new BlockReceiver(block[index], in,
	    			s.getRemoteSocketAddress().toString(),
	    			s.getLocalSocketAddress().toString(),
	    			isRecovery, client, srcDataNode, datanode);
	    	...
	    	replyOut = new DataOutputStream(
		      ...
		      if (targets.length > 0) {
		        ...
		        try {
		          ...
		          mirrorOut.writeShort( DataTransferProtocol.DATA_TRANSFER_VERSION );
		          mirrorOut.write( DataTransferProtocol.OP_WRITE_BLOCK );
		          mirrorOut.writeLong( block[index].getBlockId() );
		          mirrorOut.writeLong( block[index].getGenerationStamp() );
		          mirrorOut.writeInt( pipelineSize );
		          mirrorOut.writeBoolean( isRecovery );
		          Text.writeString( mirrorOut, client );
		          mirrorOut.writeBoolean(hasSrcDataNode);
		          ...
		          if (client.length() != 0) {
		        	...
		        	if (client.length() > 0) {
		        		...
		        		LOG.info(datanode.dnRegistration + ":Exception transfering block " +
		        				block[index] + " to mirror " + mirrorNode +
		        				". continuing without the mirror.\n" +
		        				StringUtils.stringifyException(e));
		        	}
		        }
		      ...
		      // the block is finalized in the PacketResponder.
		      if (client.length() == 0) {
		        datanode.notifyNamenodeReceivedBlock(block[index], DataNode.EMPTY_DEL_HINT);
		        LOG.info("Received block " + block[index] +
		                 " src: " + remoteAddress +
		                 " dest: " + localAddress +
		                 " of size " + block[index].getNumBytes());
		      }

		      if (datanode.blockScanner != null) {
		    	  datanode.blockScanner.addBlock(block[index]);
		      }

    	} catch (IOException ioe) {
    		LOG.info("writeBlock " + block[index] + " received exception " + ioe);
    		throw ioe;
    	}
    	finally {
	      // close all opened streams
