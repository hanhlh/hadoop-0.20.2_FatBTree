 ...
 * regularly with a single NameNodeFBT.  It also communicates
 * with client code and other DataNodes from time to time.
 *
 * DataNodes store a series of named blocks.  The DataNode
 * allows client code to read these blocks, or to write new
 * block data.  The DataNode may also, in response to instructions
 * from its NameNodeFBT, delete blocks or copy blocks to/from other
 * DataNodes.
 *
 ...
 * reports the table's contents to the NameNodeFBT upon startup
 * and every so often afterwards.
 *
 * DataNodes spend their lives in an endless loop of asking
 * the NameNodeFBT for something to do.  A NameNodeFBT cannot connect
 * to a DataNode directly; a NameNodeFBT simply returns values from
 * functions invoked by a DataNode.
 *
 * DataNodes maintain an open server socket so that client code
 * or other DataNodes can read/write data.  The host/port for
 * this server is reported to the NameNodeFBT, which then sends that
...
public class DataNode extends Configured
  ...
  // Default port for ClientProtocol defining file system metadata operations
  ...
  //append
  public ArrayList<DatanodeProtocol> namenodes = new ArrayList<DatanodeProtocol>();
  ...
  ThreadGroup threadNamespaceGroup = null;
  ...
  private static InetSocketAddress[] nameNodeAddrs;
...
void startDataNode(Configuration conf,
  ...
  /** Number of concurrent namespaceXceivers per node. */
  int getNamespaceXceiverCount() {
    return threadNamespaceGroup == null ? 0 : threadNamespaceGroup.activeCount();
  }
    ...
    while (shouldRun) {
      try {
        ...
        if (blockArray != null) {
          ...
          LOG.info("connect to namenode "+namenode.toString());
        ...
        //System.out.println("DataNode.offerService received block sent to NN");
        // send block report
  ...
  private boolean processCommand(DatanodeCommand cmd) throws IOException {
    ...
    //append
    case DatanodeProtocol.DNA_TRANSFERS:
    //Send a copy of multiple blocks to an another datanode
    System.out.println("target,"+Arrays.toString(bcmd.getTargets()[0]));
    transferMultipleBlocks(bcmd.getBlocks(), bcmd.getTargets()[0]);
  ...
  private void transferBlock( Block block,
    ...
    if (block.getNumBytes() > onDiskLength) {
      ...
      LOG.info("Can't replicate block " + block
          ...
          + " is shorter than NameNodeFBT recorded length " + block.getNumBytes());
      return;
    }
  ...
  /**
   * Transfer multiple blocks to single DataNode
 * @throws IOException
   * */

  private void transferMultipleBlocks(Block[] blocks,
		  				DatanodeInfo xferTargets[]) throws IOException {
	  LOG.info("transferMultipleBlocks to "+xferTargets[0]);
	 ...
	 if (numTargets > 0) {
		 if (LOG.isInfoEnabled()) {
			 ...
			 LOG.info(dnRegistration + " Starting thread to transfer blocks  to " + xfersBuilder);
	      	}

		 new Daemon(new DataTransferMultipleBlocks(xferTargets, blocks, this)).start();
	 }
   ...
   * That's the loop that connects to the NameNodeFBT and provides basic DataNode
   * functionality.
   *
  ...
  public static DataNode instantiateDataNode(String args[],
    ...
    if (conf.get("dfs.network.script") != null) {
      LOG.error("This configuration for rack identification is not supported" +
          " anymore. RackID resolution is handled by the NameNodeFBT.");
      System.exit(-1);
    }
    ...
    for(int i=0; i < argsLen; i++) {
      ...
      if ("-r".equalsIgnoreCase(cmd) || "--rack".equalsIgnoreCase(cmd)) {
        LOG.error("-r, --rack arguments are not supported anymore. RackID " +
            "resolution is handled by the NameNodeFBT.");
        System.exit(-1);
  ...
  public Daemon recoverBlocks(final Block[] blocks, final DatanodeInfo[][] targets) {
    Daemon d = new Daemon(threadGroup, new Runnable() {
      ...
      public void run() {
        for(int i = 0; i < blocks.length; i++) {
          try {
            logRecoverBlock("NameNodeFBT", blocks[i], targets[i]);
  ...
  /**
   * Send multiple blocks inside one oppened connection
   * For transfering data when shifing from Low Gear to High Gear
   * */

  class DataTransferMultipleBlocks implements Runnable {
    DatanodeInfo targets[];
    Block[] blocks;
    DataNode datanode;
     ...
     * entire target list, the block, and the data.
     */
    public DataTransferMultipleBlocks(DatanodeInfo targets[], Block[] blocks,
    ...
    public void run() {
      ...
      try {
        ...
        blockSender = new BlockSender[numBlocks];
        out = new DataOutputStream(new BufferedOutputStream(baseStream,
                SMALL_BUFFER_SIZE));
        ...
        //
        out.writeShort(DataTransferProtocol.DATA_TRANSFER_VERSION);
        out.writeByte(DataTransferProtocol.OP_WRITE_BLOCKS);
        out.writeInt(numBlocks);
        int i=0;
        for (Block b:blocks) {
        	LOG.info("send block "+b);
        	blockSender[i] = new BlockSender(b, 0, b.getNumBytes(), false, false, false,
        			datanode);
        	DatanodeInfo srcNode = new DatanodeInfo(dnRegistration);
        	out.writeLong(b.getBlockId());
	        out.writeLong(b.getGenerationStamp());
	        ...
	        // write targets
	        out.writeInt(targets.length - 1);
	        for (int j = 1; i < targets.length; j++) {
	          targets[j].write(out);
	        }
	        // send data & checksum
	        blockSender[i].sendBlock(out, baseStream, null);
        	...
        	LOG.warn(dnRegistration + ":Failed to transfer blocks to " + targets[0].getName()
        			+ " got " + StringUtils.stringifyException(ie));
      }
        finally {
	        xmitsInProgress.getAndDecrement();
