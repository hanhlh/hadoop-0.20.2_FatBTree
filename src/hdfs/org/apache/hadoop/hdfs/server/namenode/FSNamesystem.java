...
import metrics.FSNamesystemMetrics;
...
public class FSNamesystem implements FSConstants, FSNamesystemMBean,
  ...
  protected static final void logAuditEvent(UserGroupInformation ugi,
  ...
  //
  public BlocksMap blocksMap = new BlocksMap(DEFAULT_INITIAL_MAP_CAPACITY,
                                            DEFAULT_MAP_LOAD_FACTOR);

  //
  // Store blocks-->datanodedescriptor(s) map of corrupt replicas
  //
  public transient CorruptReplicasMap corruptReplicas = new CorruptReplicasMap();
...
//appended //
   ...
   */
  protected transient NavigableMap<String, DatanodeDescriptor> datanodeMap =
  ...
  //
  protected Map<String, Collection<Block>> excessReplicateMap =
    new TreeMap<String, Collection<Block>>();
   ...
   */
  transient ArrayList<DatanodeDescriptor> heartbeats = new ArrayList<DatanodeDescriptor>();
  ...
  // Set of: Block
  //
  protected transient UnderReplicatedBlocks neededReplications = new UnderReplicatedBlocks();
  public transient PendingReplicationBlocks pendingReplications;

  public transient LeaseManager leaseManager = new LeaseManager(this);

  //
  // Threaded object that checks to see if we have been
  // getting heartbeats from all clients.
  //
  transient Daemon hbthread = null;   // HeartbeatMonitor thread
  public transient Daemon lmthread = null;   // LeaseMonitor thread
  transient Daemon smmthread = null;  // SafeModeMonitor thread
  public transient Daemon replthread = null;  // Replication thread

  private volatile boolean fsRunning = true;
  long systemStart = 0;

  //  The maximum number of replicates we should allow for a single block
  private transient int maxReplication;
  //  How many outgoing replication streams a given node should have at one time
  private transient int maxReplicationStreams;
  // MIN_REPLICATION is how many copies we need in place or else we disallow the write
  private transient int minReplication;
  // Default replication
  private transient int defaultReplication;
  // heartbeatRecheckInterval is how often namenode checks for expired datanodes
  private transient long heartbeatRecheckInterval;
  // heartbeatExpireInterval is how long namenode waits for datanode to report
  // heartbeat
  private transient long heartbeatExpireInterval;
  //replicationRecheckInterval is how often namenode checks for new replication work
  private transient long replicationRecheckInterval;
  // default block size of a file
  private transient long defaultBlockSize = 0;
   ...
   */
  private transient int replIndex = 0;
  private transient long missingBlocksInCurIter = 0;
  private transient long missingBlocksInPrevIter = 0;

  public static FSNamesystem fsNamesystemObject;
  /** NameNode RPC address */
  private transient InetSocketAddress nameNodeAddress = null; // TODO: name-node has this field, it should be removed here
  transient SafeModeInfo safeMode;  // safe mode information
  private transient Host2NodesMap host2DataNodeMap = new Host2NodesMap();

  // datanode networktoplogy
  public transient NetworkTopology clusterMap = new NetworkTopology();
  public transient DNSToSwitchMapping dnsToSwitchMapping;

  // for block replicas placement
  transient ReplicationTargetChooser replicator;

  public transient HostsFileReader hostsReader;
  private transient Daemon dnthread = null;

  private transient long maxFsObjects = 0;          // maximum number of fs objects
  ...
  private transient  int blockInvalidateLimit = FSConstants.BLOCK_INVALIDATE_CHUNK;

  // precision of access times.
  private long accessTimePrecision = 0;

  protected transient  Map<String[], String> datanodeRangeMapping;
  protected static transient int _numOfGear;
  private static transient  Map<Integer, ArrayList<String>> _gearActivateNodes;
  public static transient  Map<String, String> _WOLMapping;
  public static transient  Map<String, String[]> _backUpMapping;
  public static transient  int low;
  public static transient  int high;
  public static transient  String experimentDir;
  private transient  AtomicInteger _wolSequence = new AtomicInteger(0);
  private String namespaceOwner;

  /**
   * FSNamesystem constructor.
   */
  public FSNamesystem(NameNode nn, Configuration conf) throws IOException {
	  if (NameNode.namenodeType.equals("FBT")) {
		  //DO nothing
		  System.out.println("FSNamesystem namenodetypeFBT");
	  } else{
		  try {
			  numOfFiles = 0;// appended
			  this.namespaceOwner = "";
			  initialize(nn, conf);
		  } catch(IOException e) {
  ...
  public FSNamesystem(NameNode nn, Configuration conf, String namespaceOwner)
  			throws IOException {
   ...
   */
  protected void initialize(NameNode nn, Configuration conf) throws IOException {
	  StringUtility.debugSpace("FSNamesystem.initialize");
    this.systemStart = now();

    setDatanodeRangeMapping(conf);
	  setGear(conf);
	  setWriteOffLoadingTarget(conf);
	  setBackUpMapping(conf);

	  setConfigurationParameters(conf);
	  if (replicator instanceof ReplicationTargetChooserHashDataPlacement) {
		  ((ReplicationTargetChooserHashDataPlacement) replicator).setBackUpMapping(_backUpMapping);
		  ((ReplicationTargetChooserHashDataPlacement) replicator).setCurrentGear(1);
		  ((ReplicationTargetChooserHashDataPlacement) replicator).setDatanodeRangeMapping(datanodeRangeMapping);
		  ((ReplicationTargetChooserHashDataPlacement) replicator).setGearToActivateNodeMapping(getGearActivateNodes());
	  }
    ...
    //TODO loadFSImage form its own namespaceDirs
    this.dir.loadFSImage(getNamespaceDir(conf),
                         getNamespaceEditDir(conf), startOpt);
  ...
  public void setDatanodeRangeMapping(Configuration conf) {
	  conf.addResource("DataPlacement.xml");
	  conf.addResource("hdfs-site-fbt.xml");
	  experimentDir = conf.get("dfs.nnbenchmark.dir");
	  low = conf.getInt("dfs.nnbenchmark.low", 0);
	  high = conf.getInt("dfs.nnbenchmark.high", 1000);

	//datanodesList = edn13, edn14, edn15, edn16
	    datanodeRangeMapping = new HashMap<String[], String>();
		  String[] datanodes = conf.getStrings("dfs.datanodes");
		  int id=1;
		  for (String datanode:datanodes) {
			  String[] range= new String[2];
			  String start = StringUtility.generateRange(id,
				  		datanodes.length,
				  		experimentDir,
				  		low,
				  		high)[0];
			  ...
			  range[0] = start;
			  String end = StringUtility.generateRange(id,
					  		datanodes.length,
					  		experimentDir,
					  		low,
					  		high)[1];
			  ...
			  range[1]=end;
			  datanodeRangeMapping.put(range, datanode);
			  id++;
		  }

  }
  public void setBackUpMapping(Configuration conf) {
	  Configuration.addDefaultResource("DataPlacement.xml");
	  _backUpMapping = new ConcurrentHashMap<String, String[]>();

	  for (String datanode:conf.getStrings("fs.namenodeFBTs")) {
		  String[] backUpNodes = conf.getStrings("backUpPlacement."+datanode);
		  System.out.println(datanode+","+Arrays.toString(backUpNodes));
		  _backUpMapping.put(datanode, backUpNodes);
	  }
  }

  public Map<String, String[]> getBackUpMapping() {
	  return _backUpMapping;
  }

  public void setGear(Configuration conf) {
		//numOfGear>=1
	  Configuration.addDefaultResource("gear.xml");


		_numOfGear = conf.getInt("gear.numberOfGear", 1);
		if (_numOfGear<1) {
			System.err.print("wrong numOfGear");
		}
		setGearActivateNodes(new ConcurrentHashMap<Integer,ArrayList<String>>());
		for (int i=1; i<=_numOfGear; i++) {
			...
			getGearActivateNodes().put(i, activateNodesL);
		}
		System.out.println("gearActivateNode: "+getGearActivateNodes().size());
  }

  public void setWriteOffLoadingTarget(Configuration conf) {
	  Configuration.addDefaultResource("WOLLocation.xml");
	  _WOLMapping = new ConcurrentHashMap<String, String>();
		NameNodeFBT.fsNameNodeFBTs = conf.getStrings("fs.namenodeFBTs");
		for (String hostname:NameNodeFBT.fsNameNodeFBTs) {
			String targetWOLName = conf.get("WOLPlacement."+hostname);
			_WOLMapping.put(hostname, targetWOLName);
		}
  }
  ...
  public static Collection<File> getNamespaceDirs(Configuration conf) {
    ...
    if (NameNode.namenodeType.equals("FBT") ||
    		NameNode.namenodeType.equals("StaticPartition")) {
	    conf.addResource("gear.xml");
	    String[] backupNodes = conf.getStrings("fbt.backupNode", null);
	    String primaryNode = conf.get("fbt.primaryNode", null);
	    for(String name : dirNames) {
	    	for (String backupNode : backupNodes) {
	    		dirs.add(new File(name.concat(".").concat(backupNode)));
	    	}
	    	dirs.add(new File(name.concat(".").concat(primaryNode)));
	    }
    }
    for (String name:dirNames) {
  ...
  public static Collection<File> getNamespaceEditsDirs(Configuration conf) {
    ...
    if (NameNode.namenodeType.equals("FBT") ||
    		NameNode.namenodeType.equals("StaticPartition")) {
	    String[] backupNodes = conf.getStrings("fbt.backupNode");
	    String primaryNode = conf.get("fbt.primaryNode");
    ...
    for (File f:dirs) {
    	System.out.println("editsDir: "+f.getName());
    }
    return dirs;
  }

  public Collection<File> getNamespaceDir(Configuration conf) {
	    ...
	    Collection<File> dirs = new ArrayList<File>(dirNames.size());
	    for (String name:dirNames) {
	    	if (!getNamespaceOwner().equals("")) { //not normal HDFS
	    			dirs.add(new File(name.concat("."+getNamespaceOwner())));
	    	} else {
	    		dirs.add(new File (name));
	    	}
	    }
  ...
  public  Collection<File> getNamespaceEditDir(Configuration conf) {
    Collection<String> editsDirNames =
            conf.getStringCollection("dfs.name.edits.dir");
    if (editsDirNames.isEmpty())
      editsDirNames.add("/tmp/hadoop/dfs/name");
   ...
   * is stored
   */
  protected FSNamesystem(FSImage fsImage, Configuration conf) throws IOException {
	  if (NameNode.namenodeType.equals("FBT")) {
  ...
  public void setConfigurationParameters(Configuration conf)
    ...
    //append
    if (conf.get("dfs.dataplacementType").equals("HashRange")) {
    	System.out.println("HashRange");
    	this.replicator = new ReplicationTargetChooserHashDataPlacement(
...
/**
  ...
  synchronized void metaSave(String filename) throws IOException {
    ...
    synchronized (neededReplications) {
      ...
      for (Block block : neededReplications) {
        ...
        // l: == live:, d: == decommissioned c: ==  e: == excess
  ...
  protected synchronized void updateNeededReplications(Block block,
                        int curReplicasDelta, int expectedReplicasDelta) {
	  NameNode.LOG.info("FSNamesystem.updateNeededReplication");
 ...
 * @throws MessageException
  ...
  LocatedBlocks getBlockLocations(String clientMachine, String src,
      long offset, long length) throws IOException, MessageException {
 ...
 * @throws MessageException
   ...
   */
  public LocatedBlocks getBlockLocations(String src, long offset, long length,
      boolean doAccessTime) throws IOException, MessageException {
    if (offset < 0) {
  ...
  private synchronized LocatedBlocks getBlockLocationsInternal(String src,
    ...
    if (blocks == null) {
    	//append for open INode only workload
    	LocatedBlocks lbs = new LocatedBlocks();
    	return lbs;
    	//end append
      //return null;
    }
    ...
    ArrayList<LocatedBlock> results;
 ...
 * @throws MessageException
   */
  public synchronized void setTimes(String src, long mtime, long atime) throws IOException, MessageException {
  ...
  private synchronized void startFileInternal(String src,
    ...
    try {
      ...
      if (append) {
       ...
       //
       //checkFsObjectLimit(); //This function is not implemented in FBT
 ...
 * @throws MessageException
  ...
  public LocatedBlock getAdditionalBlock(String src,
    ...
    if (replicator instanceof ReplicationTargetChooserHashDataPlacement) {
    	...
    	((ReplicationTargetChooserHashDataPlacement) replicator).setSrc(src);
    	targets = ((ReplicationTargetChooserHashDataPlacement) replicator).chooseTarget(1,
                clientNode,
                //null,
                excludedNodes,
                blockSize);
    	...
    	targets = replicator.chooseTarget(1,
                                                           clientNode,
                                                           //null,
                                                           excludedNodes,
                                                           blockSize);
  ...
  public CompleteFileStatus completeFile(String src, String holder) throws IOException, MessageException {
  ...
  private synchronized CompleteFileStatus completeFileInternal(String src,
                                                String holder) throws IOException, MessageException {
  ...
  //Perform WriteOffloading
    //TODO getGearInformation from Gearm Manager
		//do WriteOffLoading writting if _directory.getOwner is in list
      System.out.println("Implement write wol blocks");
		String wolTarget = ((ReplicationTargetChooserHashDataPlacement) replicator).getDatanodeName(src);
		String wolSource = ((ReplicationTargetChooserHashDataPlacement) replicator).getDatanodeName(src, 1);
		System.out.println("owner "+wolTarget);

  	if (!getGearActivateNodes().get(1).contains(wolTarget)) {
  		writeOffloading(wolTarget, wolSource, src, fileBlocks);
  	}
   ...
   */
  public void checkReplicationFactor(INodeFile file) {
	  //NameNode.LOG.info("checkReplicationFactor for "+file+" starts");
    ...
    NameNode.LOG.info("checkReplicationFactor for "+file+" ended");
  }

  public static Random randBlockId = new Random();
 ...
 * @throws MessageException
  ...
  public synchronized void commitBlockSynchronization(Block lastblock,
  ...
  protected void setDatanodeDead(DatanodeDescriptor node) throws IOException {
  ...
  public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
    ...
    synchronized (heartbeats) {
      ...
      //check pending replication when shifting gear
        //System.out.println("FSNamesystem.getReplicationCommand()");
    ...
    public void run() {
      while (fsRunning) {
        try {
          ...
          LOG.info("ReplicationMonitor thread received Runtime exception. " + t);
  ...
  public int getNodeReplicationLimit() {
  ...
  //comment out by hanhlh begin
  //FSImage getFSImage() {
  //  return dir.fsImage;
  //}
  //comment out by hanhlh end
  //add by hanhlh begin
  ...
  //add by hanhlh end
  protected FSEditLog getEditLog() {
...
*/    return block;
   ...
   */
  protected void invalidateCorruptReplicas(Block blk) {
   ...
   */
  protected void processOverReplicatedBlock(Block block, short replication,
  ...
  public static class NumberReplicas {
    ...
    public NumberReplicas(int live, int decommissioned, int corrupt, int excess) {
   ...
   */
  protected boolean shouldNodeShutdown(DatanodeDescriptor node) {
   ...
   */
     ...
     */
    public SafeModeInfo(Configuration conf) {
   ...
   * @param replication current replication
   */
  protected void incrementSafeBlockCount(int replication) {
    if (safeMode == null)
      return;
  ...
  transient UpgradeManagerNamenode upgradeManager = new UpgradeManagerNamenode();
  ...
  protected int getDistributedUpgradeVersion() {
  ...
  protected PermissionChecker checkPathAccess(String path, FsAction access
      ) throws AccessControlException {
  ...
  protected PermissionChecker checkAncestorAccess(String path, FsAction access
      ) throws AccessControlException {
    return checkPermission(path, false, access, null, null, null);
  }

  public PermissionChecker checkTraverse(String path
      ) throws AccessControlException {
   ...
   */
  protected void registerMBean(Configuration conf) {
   ...
   * Increments, logs and then returns the stamp
   */
  public synchronized long nextGenerationStampForBlock(Block block, boolean fromNN)
  						throws IOException {
    BlockInfo storedBlock = blocksMap.getStoredBlock(block);
    if (storedBlock == null) {
  ...
  public void setNamenodeID(int nameID) {
	// TODO ��ư�������줿�᥽�åɡ�������
		  namenodeID = nameID;
	  }
// end of appended //
...
public void writeOffloading(String wolTarget, String wolSource,
	...
	StringUtility.debugSpace("src, wolTarget, wolSource: "+src+","+wolTarget+","+wolSource);
	Date startTime = new Date();
	String wolDestination = _WOLMapping.get(wolTarget);
	System.out.println("wolDestination: "+wolDestination);
	DatanodeDescriptor WOLSource = (DatanodeDescriptor) clusterMap.
		getNode("/default-rack/192.168.0."+
				(100+Integer.parseInt(wolSource.substring(wolSource.length()-2)))+
				":50010");
	DatanodeDescriptor WOLDestination = (DatanodeDescriptor) clusterMap.
		getNode("/default-rack/192.168.0."+
				(100+Integer.parseInt(wolDestination.substring(wolDestination.length()-2)))+
				":50010");
	...
	System.out.println("src, wolTarget, wolSource, wolDestination, response: "+src+","+wolTarget+","+wolSource
						+","+wolDestination+
	...
	private void writeOffloadingLogging(String src, String WOLDestination,
		...
		wol.outputRecord(record, nameNodeAddress.getHostName());
	}
	public int getWOLCounting() {
		return getWOLSequence().getAndIncrement();
	}

	public AtomicInteger getWOLSequence() {
		return _wolSequence;
	}

	public INodeFile getINodeFile(String src) {
		return dir.getFileINode(src);
	}


	public static void setGearActivateNodes(Map<Integer, ArrayList<String>> _gearActivateNodes) {
		FSNamesystem._gearActivateNodes = _gearActivateNodes;
	}


	public static Map<Integer, ArrayList<String>> getGearActivateNodes() {
		return _gearActivateNodes;
	}
	...
	public void setHeartbeat(ArrayList<DatanodeDescriptor> heartbeats) {
		  this.heartbeats = heartbeats;
	}

	public ArrayList<DatanodeDescriptor> getHeartbeats() {
		  return this.heartbeats;
	}

	public void setDatanodeMap(NavigableMap<String, DatanodeDescriptor> datanodeMap) {
		this.datanodeMap = datanodeMap;
	}

	public NavigableMap<String, DatanodeDescriptor> getDatanodeMap() {
		return this.datanodeMap;
	}

	public void setUpgradeManager(UpgradeManagerNamenode upgradeManager) {
	...
	public void setCorruptReplicasMap(CorruptReplicasMap corruptReplicas) {
		this.corruptReplicas = corruptReplicas;
	}

	public CorruptReplicasMap getCorruptReplicasMap() {
		return this.corruptReplicas;
	}
	public void setLeasManager(LeaseManager leaseManager) {
		this.leaseManager = leaseManager;
	}

	public Map<String, Collection<Block>> getRecentInvalidateSets() {
		return this.recentInvalidateSets;
	}

	public void setRecentInvalidateSets(Map<String, Collection<Block>> recentInvalidateSets) {
		this.recentInvalidateSets = recentInvalidateSets;
	}

	public Map<String, Collection<Block>> getExcessReplicateMap() {
		return this.excessReplicateMap;
	}
	public void setExcessReplicateMap(Map<String, Collection<Block>> excessReplicateMap) {
		this.excessReplicateMap = excessReplicateMap;
	}
