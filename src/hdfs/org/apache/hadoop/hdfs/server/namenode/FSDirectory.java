 ...
 *
 *************************************************/
public class FSDirectory implements FSConstants, Closeable, Serializable {
  ...
  public final INodeDirectoryWithQuota rootDir;
  //public
  public transient FSImage fsImage;
  private boolean ready = false;
  //append
  final FBTDirectory directory;
  //private final int lsLimit; // max list limit

  private Map<Integer, InetSocketAddress> nnAddrs;

  public void setNNAddrs(Map<Integer, InetSocketAddress> addrs) {
		nnAddrs = addrs;
  }
  ...
  public FSDirectory(FSNamesystem ns, Configuration conf) {
    ...
    fsImage.setCheckpointDirectories(FSImage.getCheckpointDirs(conf, null, ns.getNamespaceOwner()),
                                FSImage.getCheckpointEditsDirs(conf, null, ns.getNamespaceOwner()));
  }

  /** Access an existing dfs name directory. */
  public FSDirectory(FBTDirectory fbtd, Configuration conf, String owner) {
    this(new FSImage(), fbtd, conf);
    fsImage.setCheckpointDirectories(FSImage.getCheckpointDirs(conf, null, owner),
                                FSImage.getCheckpointEditsDirs(conf, null, owner));
  }
  ...
  public FSDirectory(FSImage fsImage, FSNamesystem ns, Configuration conf) {
    ...
    rootDir = new INodeDirectoryWithQuota(INodeDirectory.ROOT_NAME,
			        ...
			        Integer.MAX_VALUE, -1);
    // appended //
	nnAddrs = new TreeMap<Integer, InetSocketAddress>();
	int nID = conf.getInt("dfs.namenode.id", 0);
	InetSocketAddress nAddr = NetUtils.createSocketAddr(
			conf.get("fs.default.name"), 8020);
	nnAddrs.put(new Integer(nID), nAddr);
	rootDir.setNamenodeID(nID);
	rootDir.addINodeLocation(nID);
	// end of appended
  ...
  public FSDirectory(FSImage fsImage, FBTDirectory fbtd, Configuration conf) {
	    rootDir = new INodeDirectoryWithQuota(INodeDirectory.ROOT_NAME,
	        fbtd.createFsOwnerPermissions(new FsPermission((short)0755)),
	        Integer.MAX_VALUE, -1);
	    this.fsImage = fsImage;
	    namesystem = null;
	    initialize(conf);
  ...
  public void loadFSImage(Collection<File> dataDirs,
                   Collection<File> editsDirs,
                   StartupOption startOpt) throws IOException {
	  StringUtility.debugSpace("FSDirectory.loadFSImage");
	  System.out.println("startOpt "+ startOpt.name());
  ...
  long getPreferredBlockSize(String filename) throws IOException {
    synchronized (rootDir) {
      //INode fileNode = rootDir.getNode(filename);
    	INode fileNode = rootDir.getNode(filename, nnAddrs, false);
      if (fileNode == null) {
  ...
  boolean isDirEmpty(String src) {
    ...
    synchronized(rootDir) {
      //INode targetNode = rootDir.getNode(src);
    	INode targetNode = rootDir.getNode(src, nnAddrs, false);
  ...
  FileStatus[] getListing(String src) {
    ...
    synchronized (rootDir) {
      //INode targetNode = rootDir.getNode(srcs);
    	INode targetNode = rootDir.getNode(srcs, nnAddrs, false);
      if (targetNode == null)
        return null;
  ...
  Block[] getFileBlocks(String src) {
    ...
    synchronized (rootDir) {
      //INode targetNode = rootDir.getNode(src);
      INode targetNode = rootDir.getNode(src, nnAddrs, false);
      NameNode.LOG.info("targetNode "+targetNode);
      if (targetNode == null) {
    	  return null;
      }
      if(targetNode.isDirectory())
  ...
  boolean isValidToCreate(String src) {
    ...
    synchronized (rootDir) {
      if (srcs.startsWith("/") &&
          ...
          rootDir.getNode(srcs, nnAddrs, false) == null) {
        return true;
      } else {
        return false;
      }
  ...
  boolean isDir(String src) {
    synchronized (rootDir) {
      //INode node = rootDir.getNode(normalizePath(src));
    	INode node = rootDir.getNode(normalizePath(src), nnAddrs, false);
  ...
  public <T extends INode> T addNode(String src, T child,
   ...
   */
  public void setTimes(String src, INodeFile inode, long mtime, long atime, boolean force)
                                                        throws IOException {
  ...
  private boolean unprotectedSetTimes(String src, INodeFile inode, long mtime,
    ...
    if (atime != -1) {
      ...
      if (!NameNode.namenodeType.equals("FBT")) {
    	  ...
    	  if (atime <= inodeTime + directory.getAccessTimePrecision() && !force) {
  	        status =  false;
  	      } else {
  	        inode.setAccessTime(atime);
  	        status = true;
...
// appended //
	public INode getNodeFromOther(byte[][] components, List<Integer> visited) {
		return rootDir.getNodeFromCluster(components, nnAddrs, visited);
	}

	private void sendCopying(String src, int id, List<Integer> iLoc, long accessTime, long modificationTime) {
		Configuration conf = new Configuration();
		int i;
		for (Iterator<Integer> it = iLoc.iterator(); it.hasNext();) {
			i = it.next().intValue();
			if (!(id == i)) {
				...
				try {
					namenode = (NNClusterProtocol) RPC.waitForProxy(
							...
							nnAddrs.get(new Integer(i)), conf);
					namenode.setCopying(src, id, accessTime, modificationTime);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	public void setCopying(String src, int id, long atime, long mtime) {
		INode setNode = rootDir.getNode(src, nnAddrs, false);
		setNode.addINodeLocation(id);
		setNode.setAccessTime(atime);
		setNode.setModificationTime(mtime);
	}
	...
	public String searchEnd() {
		...
		int next;
		String endPath = "/";
		while(curNode instanceof INodeDirectory) {
			if(((INodeDirectory) curNode).getChildren() == null || ((INodeDirectory) curNode).getChildren().isEmpty())
				return endPath;
			Collections.sort(((INodeDirectory) curNode).getChildren(), new INodeComparator());
			next = ((INodeDirectory) curNode).getChildren().size() - 1;
			childNode = ((INodeDirectory) curNode).getChildren().get(next);
			endPath += DFSUtil.bytes2String(childNode.name);
			curNode = childNode;
		}
		return endPath;
	}

	private class INodeComparator implements Comparator<INode> {

		public int compare(INode o1, INode o2) {
			// TODO Auto-generated method stub
			return INode.compareBytes(o1.name, o2.name);
		}
	...
	public String searchStart() {
		...
		while(curNode instanceof INodeDirectory) {
			...
			next = 0;
			childNode = ((INodeDirectory) curNode).getChildren().get(next);
			startPath += DFSUtil.bytes2String(childNode.name);
			curNode = childNode;
		}
		return startPath;
	}
