...
public class INodeFileUnderConstruction extends INodeFile {
  ...
  public void setTargets(DatanodeDescriptor[] targets) {
  ...
  public synchronized void setLastBlock(BlockInfo newblock, DatanodeDescriptor[] newtargets
      ) throws IOException {
    if (blocks == null) {
...
//appended//
  ...
  public INodeFileUnderConstruction(INodeFileUnderConstructionInfo info) {
  	super(info);
  	this.clientName = info.getClientName();
  	this.clientMachine = info.getClientMachine();
  	this.clientNode = info.getClientNode();
  	this.primaryNodeIndex = info.getPrimaryNodeIndex();
  	this.targets = info.getTargets();
  	this.lastRecoveryTime = info.getLastRecoveryTime();
  }
