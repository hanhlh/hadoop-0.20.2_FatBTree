...
public class DatanodeDescriptor extends DatanodeInfo implements Serializable{
  ...
  private static class BlockQueue implements Serializable{
  ...
  /** A BlocksTargetPair queue. */
  private static class BlocksQueue implements Serializable{
    private final Queue<BlocksTargetPair> blockq =
    				new LinkedList<BlocksTargetPair>();

    /** Size of the queue */
    synchronized int size() {return blockq.size();}

    /** Enqueue */
    synchronized boolean offer(Block[] blocks, DatanodeDescriptor[][] targets) {
      return blockq.offer(new BlocksTargetPair(blocks, targets));
    }

    /** Dequeue */
    synchronized List<BlocksTargetPair> poll(int numBlocks) {
      if (numBlocks <= 0 || blockq.isEmpty()) {
        return null;
      }

      List<BlocksTargetPair> results = new ArrayList<BlocksTargetPair>();
      for(; !blockq.isEmpty() && numBlocks > 0; numBlocks--) {
        results.add(blockq.poll());
      }
      return results;
  ...
  /** A BlocksTarget constructor*/
  public static class BlocksTargetPair implements Serializable {
	    public final Block[] blocks;
	    public final DatanodeDescriptor[][] targets;

	    public BlocksTargetPair(Block[] blocks, DatanodeDescriptor[][] targets) {
	      this.blocks = blocks;
	      this.targets = targets;
	    }
		...
		public String toString() {
			return "BlocksTargetPair [blocks=" + Arrays.toString(blocks)
					+ ", targets=" + Arrays.toString(targets[0]) + "]";
		}
  ...
  private BlocksQueue replicateBlocksWOL = new BlocksQueue();
   ...
   */
  public void addBlockToBeReplicated(Block block, DatanodeDescriptor[] targets) {
  ...
  //append
  BlockCommand getReplicationCommand() {
	  List<BlocksTargetPair> blocksTargetPairs = replicateBlocksWOL.poll(1);
	  return blocksTargetPairs == null? null:
		  new BlockCommand(DatanodeProtocol.DNA_TRANSFERS,
				  	blocksTargetPairs.get(0));
  }
  //end append
  ...
  //add
  public boolean getIsWOLTarget() {
	  return isWOLTarget;
  }

  public void setIsWOLTarget(boolean isWOL) {
	  isWOLTarget = isWOL;
  }
  //add
}
